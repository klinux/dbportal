...
Returns one page of audit events, newest first (docs/CONTEXT.md §4.27). Optional query params: `type`,
`actor` (the event's user), `connection` (the datasource's name), `result` (`success` | `failure`), `from`
and `to` (ISO instants, inclusive), `limit` (default 100, at most 500) and `offset` (default 0); a malformed
one is `400`. Response: `{ "events": [], "total": <how many the filters match>, "limit", "offset", "source":
"store" | "buffer" }`. `POST /api/admin/audit` appends an event (user auto-filled from the session).
...
values to paste into the file. `400` for a malformed body, `403` for a datasource of any other engine, `404` for
...
| `DBPORTAL_AGENT_ENABLED` | No | The agent's explicit **off**-switch. Availability is otherwise derived from the AI configuration and a writable ledger — see [`docs/AGENT.md`](AGENT.md) |
