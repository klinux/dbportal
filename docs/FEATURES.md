...
*   **SQL Databases:**
    ...
    *   **Amazon Athena:** Full query support against a **serverless** service, reached through the official AWS SDK (`@aws-sdk/client-athena`, pure JavaScript) with an access key pair or the runtime's own role. A statement is a job: submitted, polled to a terminal state, its rows read page by page, and **stopped on every exit path** - an abandoned poll does not stop a job, and a job is billed for every byte it scans. The connection names a region, a workgroup and the S3 prefix results land in; the schema tree comes from the Glue Data Catalog through the service's metadata API rather than from statements, so a refresh costs no job and writes nothing to S3, and a folder over 10,000 tables is badged as a floor rather than cut. Monitoring is the workgroup's own execution history - engine time, queue time and bytes scanned per statement - and `kill` stops a statement by id. What it declares as absent is absent from the engine: no keys, no indexes, no uptime, no storage figures, no Create Table from the modal (a Hive table needs a `LOCATION`), and no EXPLAIN until the JSON plan is measured against the live service.
