...
### 10. Roles: the agent surface run apart

`role: agent` (docs/CONTEXT.md §4.30) writes `DBPORTAL_ROLE=agent`, and the same image then serves only the service API (`/api/v1`), the MCP endpoint (`/api/mcp`), the probes and the metrics scrape; every page, session route and admin route is a `404`. `role: worker` (§4.40) writes `DBPORTAL_ROLE=worker`: the instance serves only the probes and the scrape and runs the job queue. `keda.enabled` on that release renders a KEDA `ScaledObject` whose Prometheus trigger reads `dbportal_jobs_queued` from `/api/metrics` and keeps `keda.queuedPerReplica` queued jobs per worker (`templates/scaledobject.yaml`; refused on another role, together with the HPA, or with the queue outside PostgreSQL; the Deployment then renders no `replicas` line, so an upgrade does not reset what the scaler chose). `config.jobsWorker: "off"` on the studio release stops its own loop once workers run apart. Files a worker writes (`EXPORT_DIR`, `BACKUP_DIR`) live on a volume both releases mount, or in the bucket. `tests/unit/helm-chart-keda.test.ts` holds the render and the refusals.

**The roles in one release** (`workers.enabled`, `agentRole.enabled`; since chart 0.1.2): the same release renders a Deployment per role from one pod template (`templates/_pod.tpl`), so a mount, a probe or a secret reference changed once reaches every role; they share the release's ConfigMap, Secret, seed and datasource secrets. Each role Deployment carries its own name label (`app.kubernetes.io/name: dbportal-worker`, `dbportal-agent`, plus `app.kubernetes.io/component`), which is what keeps its pods out of the studio's selectors - a Deployment selector is immutable and overlapping selectors fight - and gives it a Service (`<release>-agent`), a NetworkPolicy and a scaler of its own: `keda.enabled` then targets `<release>-workers` and the HPA keeps the studio. The workers mount the release's data volume (ReadWriteMany when persistent, refused otherwise); the agent gets an emptyDir, it keeps nothing. The studio writes `JOBS_WORKER=off` for itself when workers render, unless `config.jobsWorker` says otherwise. `tests/unit/helm-chart-roles.test.ts` holds it, and that a default render is byte for byte what it was. A default render writes nothing, so a studio release is byte-for-byte what it was. Run the agent role as its **own release** - its own Deployment, Service and NetworkPolicy - pointed at the same storage backend (`STORAGE_PROVIDER`, the seed ConfigMap, the same secrets for the datasources its tokens name), and give only that release's Service to the agents. `tests/unit/helm-chart-role.test.ts` holds the env variable to the value and the schema to the two words.
