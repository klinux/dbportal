...
const unrecognizedFlagMessage = (raw: string): string =>
  `dbportal: unrecognized ${AGENT_ENABLED_ENV} value "${raw}"; it is ignored and the agent's availability is derived from the AI configuration (use "false" to switch the agent off)`;
...
const unsanctionedTargetMessage = (raw: string): string =>
  `dbportal: unsupported ${AGENT_WORLD_TARGET_ENV} value "${raw}"; the agent runtime accepts only: ${ACCEPTED_TARGETS}`;

const implicitHostedWorldMessage = (): string =>
  `dbportal: ${HOSTED_DEPLOYMENT_ENV} is set with no ${AGENT_WORLD_TARGET_ENV}; set it explicitly to one of: ${ACCEPTED_TARGETS}`;
...
const unrecognizedThreadFlagMessage = (raw: string): string =>
  `dbportal: unrecognized ${AGENT_THREAD_CONTEXT_ENV} value "${raw}"; it is ignored and conversation context stays on (use "false" to switch it off)`;
