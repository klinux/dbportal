...
import { EnvironmentError } from "@/lib/environments/store";
import { logger } from "@/lib/logger";

/** The store's own refusals answered with their status; anything else through the shared mapper. */
export function answerEnvironmentError(error: unknown, route: string): NextResponse {
  if (error instanceof EnvironmentError) {
    logger.warn("Environment request refused", { route, statusCode: error.statusCode });
