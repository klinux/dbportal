...
interface ConnectionModalProps {
  ...
  /**
   * The caller's security inputs (who may open, write, review, export), on a tab of their own
   * beside the connection fields (docs/CONTEXT.md §4.37): a long form read as two short ones.
   */
...
export function ConnectionModal({
  ...
  const [sheetTab, setSheetTab] = useState<"connection" | "security">("connection");
  ...
  const formContent = (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        {securityFields !== undefined && (
          <Tabs value={sheetTab} onValueChange={(v) => setSheetTab(v as "connection" | "security")} className="mb-6">
            <TabsList className={ADMIN_SUBTAB_LIST_CLASS}>
              <TabsTrigger value="connection" className={ADMIN_SUBTAB_TRIGGER_CLASS} data-testid="sheet-tab-connection">
                <Plug className="h-3.5 w-3.5" />
                Connection
              </TabsTrigger>
              <TabsTrigger value="security" className={ADMIN_SUBTAB_TRIGGER_CLASS} data-testid="sheet-tab-security">
        ...
        {securityFields !== undefined && sheetTab === "security" && (
          <div data-testid="sheet-security">{securityFields}</div>
        )}
        <div hidden={securityFields !== undefined && sheetTab !== "connection"} data-testid="sheet-connection">
                ...
                <div className="space-y-1.5">
                  ...
                  {(sshProfiles ?? []).length === 0 && (
                    <p className="text-[11px] text-fg-muted">
                      No SSH profile declared yet. Profiles are declared once under Security → SSH profiles and
                      referenced here.
                    </p>
                  )}
                </div>
