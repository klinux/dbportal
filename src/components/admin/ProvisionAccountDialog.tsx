 ...
 *
 * Three steps in one dialog. On open, the datasource's own credential opens the database
 ...
 */
export function ProvisionAccountDialog({ open, onOpenChange, datasource, onProvisioned }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto" data-testid="provision-account-dialog">
        <DialogHeader>
          <DialogTitle>The portal's own account on {datasource?.name ?? "the datasource"}</DialogTitle>
          <DialogDescription>
            A least-privilege role for the portal, created with the datasource's current credential or one you type for
            this call. The plan is shown in full before anything runs; the password is generated and kept where the
            deployment keeps secrets, never shown.
          </DialogDescription>
        </DialogHeader>
          ...
          <details className="rounded-md border border-hairline p-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
              ...
              <div className="space-y-1 md:col-span-2">
                <Label htmlFor="provision-vault-mount">Vault KV mount (when the datasource names none)</Label>
                <Input
                  id="provision-vault-mount"
                  value={vaultMount}
                  onChange={(e) => setVaultMount(e.target.value)}
                  placeholder="dbportal"
                  className="h-9 text-xs font-mono"
                />
              </div>
            </div>
        ...
        <DialogFooter>
          <Button variant="outline" size="sm" onClick={() => close(false)} data-testid="provision-close">
            Close
          </Button>
          <Button variant="outline" size="sm" onClick={inspect} disabled={busy || schemas.length === 0}>
            Show the plan
          </Button>
          <Button
            size="sm"
