...
import {
  ...
  FileText,
  ClipboardCheck,
  Workflow, KeyRound } from "lucide-react";
...
export default function AdminDashboard({ children }: AdminDashboardProps) {
  ...
  const [sshIdentityOpen, setSshIdentityOpen] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => setSshIdentityOpen(true)}
              title="The SSH user and key the bastions open for you (docs/CONTEXT.md §4.9)"
            >
              <KeyRound className="mr-2 h-3.5 w-3.5" />
              <span className="hidden sm:inline">SSH identity</span>
            </Button>
            <Button
              variant="outline"
              size="sm"
      ...
      <SshIdentityDialog open={sshIdentityOpen} onOpenChange={setSshIdentityOpen} />
