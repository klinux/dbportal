...
import { BackupsPanel, formatSize } from "@/components/admin/BackupsPanel";
 ...
 * backups, a server without the tool, and a failed read.
 */
const ready = {
  supported: true,
  tool: true,
      ...
      "/api/admin/backups": (req) =>
        req.method === "POST"
          ? refuse
            ...
            : { ok: true, status: 201, json: { backup: { name: "n.dump", ...(bucket ? { object: "o" } : {}) } } }
          : { ok: true, json: ready },
    });
  ...
  test("a restore asks first, then posts the file's name; a refusal is shown in the server's words", async () => {
    ...
    const fetchMock = mockGlobalFetch({
      "/api/admin/backups/restore": () =>
        refuse ? { ok: false, status: 403, json: { error: "not offered" } } : { ok: true, json: { restored: {} } },
