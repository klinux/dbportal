 ...
 * DBPORTAL_ROLE, so a studio release is what it always was; `role: agent` or `role: worker`
 * writes the one variable that turns the same image into the surface programs call or the
 * job queue's runner; anything else is refused by the values schema. Exercises real `helm template` output.
 */
...
describe("helm chart: role", () => {
  test("a default render writes no DBPORTAL_ROLE; role=agent or role=worker writes it; another word fails the schema", () => {
    expect(render().find((e) => e.name === "DBPORTAL_ROLE")).toBeUndefined();
    expect(render("role=agent").find((e) => e.name === "DBPORTAL_ROLE")).toEqual({
      name: "DBPORTAL_ROLE",
      value: "agent",
    });
    expect(render("role=worker").find((e) => e.name === "DBPORTAL_ROLE")).toEqual({
      name: "DBPORTAL_ROLE",
      value: "worker",
    });
    expect(() => render("role=batch")).toThrow();
  });
});
